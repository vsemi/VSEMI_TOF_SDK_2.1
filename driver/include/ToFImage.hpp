...
typedef struct ToFImage
{
	...
	*/
	unsigned int* data_depth_raw;

	/*!
	* @brief Pointer of an arry float[n_points], undistorted Z data in 3D coordinates, in unit of meters.
	*
	* Example of reinterpreting to Opencv Mat:
	*
	* Mat depth_mat = Mat(tof_image->height, tof_image->width, CV_32F, tof_image->data_depth);
	*/
	...
	/*!
	* @brief Pointer of an arry uint8_t[n_points], raw grayscale data.
	*
	* Example of reinterpreting to Opencv Mat:
	*
	* Mat grayscale_mat = Mat(tof_image->height, tof_image->width, CV_8UC1, tof_image->data_grayscale);
	*/
	uint8_t* data_grayscale_raw;

	/*!
	* @brief Pointer of an arry uint8_t[n_points], undistorted grayscale data.
	*
	* Example of reinterpreting to Opencv Mat:
	*
	* Mat grayscale_mat = Mat(tof_image->height, tof_image->width, CV_8UC1, tof_image->data_grayscale);
	...
	*/
	float* data_amplitude_raw;

	/*!
	* @brief Pointer of an arry float[n_points], undistorted amplitude data.
	*
	* Example of reinterpreting to Opencv Mat:
	*
	* Mat amplitude_mat = Mat(tof_image->height, tof_image->width, CV_32F, tof_image->data_amplitude);
